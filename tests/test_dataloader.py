...
def test_download_price_vol():
    apple_price_vol = pypoanal.dataloader._download_price_volume_history('AAPL')
...
def test_download_and_save_prices():
    ...
    for ticker in tickers:
        path = pypoanal.dataloader._price_vol_path(ticker)
        assert os.path.exists(path)
    prev_mod_time = os.path.getmtime(pypoanal.dataloader._price_vol_path('AAPL'))
    ...
    # check file sizes
    assert os.path.getsize(pypoanal.dataloader._price_vol_path('AAPL')) > 400 * 1024
    assert os.path.getsize(pypoanal.dataloader._price_vol_path('GOOG')) > 150 * 1024
    # check columns
    apple_price_vol = pd.read_csv(pypoanal.dataloader._price_vol_path('AAPL'))
    assert 'Adj Close' in apple_price_vol
    assert 'Volume' in apple_price_vol


def test_get_price_vol_data():
    AAPL_price_vol_df = pypoanal.dataloader._load_price_volume_history('AAPL')
